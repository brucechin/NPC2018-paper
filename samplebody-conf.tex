...
%OLD VERSION FOR FIRST PARAGRAPH IN METHOD
%This study is based on MXNET, and BN layer is treated as an operator here. Take the forward propagation of BN layer as example, the original BN algorithm consists of only one section: normalization. In our AGBN algorithm, we propose a new global BN (GBN) algorithm to perform the communication and obtain the global mean and variance. The switch from original BN algorithm to GBN algorithm is determined by our SVM classifier, as shown in algorithm \ref{alg:reduce}.

% \begin{algorithm}[htbp] 
% \caption{AGBN algorithm} %算法的名字
% \label{alg:reduce}
% \begin{algorithmic}[1]
...
%OLD VERSION FOR SECOND PARAGRAPH IN METHOD
...
% In the GBN algorithm, we add two sections, pre-pocessing and reduce, and adjust normalization section accordingly. 
...
As we all know, we have to train a deep neural network for hundreds of epochs or even more to make it converge, which means that communication overhead between GPUs in Global BN algorithm can not be ignored. These strategies are applied to improve our algorithm.
...
Considering that we just use 8 GPUs at most for now, 
...
% NCCL\cite{jeaugey2017nccl}, The NVIDIA Collective Communications Library, implements multi-GPU and multi-node collective communication primitives which is specially optimized for NVIDIA GPUs. It provides methods like allgather/allreduce/broadcast that are optimized to achieve high bandwidth over PCIe and NVLink high-spped interconnect, and automatic topology detection is used to determine optimal communication path.Any number of GPUs ca nbe used. as long as they reside in a single node.Furthermore, NCCL implements ring-based collectives to achieve maximum bandwidth by indexing the GPUs into the optimal ring order under the hood.
